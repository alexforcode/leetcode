...
value ignoring any leading zeros. This means that revisions 1 and 001 are considered equal.
If a version number does not specify a revision at an index, then treat the revision as 0.
For example, version 1.0 is less than version 1.1 because their revision 0s are the same, but their
...
def compare_version(version1: str, version2: str) -> int:
    v1: List[int] = [int(v) for v in version1.split('.')]
    v2: List[int] = [int(v) for v in version2.split('.')]

    for val1, val2 in zip_longest(v1, v2, fillvalue=0):
        if val1 < val2:
            return -1
        elif val1 > val2:
            return 1
